export function processingTokenData(pools) {
    const tokenArray = pools.flatMap(p => [p.token_x, p.token_y]);
    const uniqueTokens = Array.from(new Map(tokenArray.map(token => [token.address, token])).values());
    const prismaTokenArray = [];
...
export function processingPoolData(pools) {
    const prismaPoolArray = [];
...
export function processingPoolSnapshotData(pools) {
    const prismaPoolSnapshotArray = [];
    for (const pool of pools) {
...
export function processingFrontendPoolData(pools) {
    const poolFrontendArray = [];
    for (const pool of pools) {
